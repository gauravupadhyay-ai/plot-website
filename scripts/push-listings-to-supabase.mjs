/**
 * Push Vrinda Vatika (and refresh Expressway) media + rows to Supabase.
 */
...
const listings = JSON.parse(fs.readFileSync(path.join(root, 'seed-media/listings.json'), 'utf8'))
...
for (const listing of listings) {
  ...
  for (let i = 0; i < (listing.localImages || []).length; i++) {
    ...
    const storagePath = `${listing.code}/${name}`
    const pub = await upload(sb, storagePath, abs)
    imageUrls.push(pub)
  ...
  for (let i = 0; i < (listing.localVideos || []).length; i++) {
    ...
    if (!fs.existsSync(abs)) {
      console.warn('  ! missing video', abs)
      continue
    }
    const ext = path.extname(abs).toLowerCase() || '.mp4'
    const storagePath = `${listing.code}/video-${String(i + 1).padStart(2, '0')}${ext}`
    const pub = await upload(sb, storagePath, abs)
    videoUrls.push(pub)
  ...
  const row = {
    ...
    area_unit: listing.areaUnit || 'sq.yd',
